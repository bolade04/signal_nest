...
def test_consolidated_bh_a_universe():
    ...
    allids = {c["id"] for c in cli.discover_collections()}
    assert len(allids) == 410  # BH-C F8: +29 discovery-recovered collections (derived/comprehension forms)
    sets = {
    ...
    assert len(allids - union) == 255  # BH-C F8: +29 (2 new SECURITY + 27 NON_SECURITY derived/internal)                 # 101 pre-BH-A + 119 B0a-SLICE2 framework + 6 BH-B assurance-validator constants
