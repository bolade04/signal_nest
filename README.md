...
- **Frontend:** 20 tests (Vitest + React Testing Library + MSW) covering the app
...
- **Backend:** 86 tests (pytest) — pure-engine unit tests for relevance, opportunity
  ...
  capability registry, versioned job contracts, tenant execution context); plus
  integration tests that drive the real FastAPI app to prove auth enforcement,
  per-location isolation, operator-gated runtime introspection, and the system
