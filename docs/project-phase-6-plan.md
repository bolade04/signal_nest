...
Lane P — PLATFORM CORRECTNESS / AUTH / DATA        (repo-only, no deps, start now)
   6B    P6-AUTH-3 ─► P6-AUTH-6   — BOTH CLOSED in 6B-1 (shared exact-role primitive
         ...
         P6-AUTH-1 — 6B-3A backend MERGED (#181); 6B-3B customer UI + role-aware
         controls complete in branch, pending merge; OPEN on main until it merges
...
| `/settings` | `pages/Settings.tsx` | yes | any | ✅ | `/auth/me`, workspaces, runtime | — | **PARTIAL** — roles card inert (`P6-UI-007`); 6B-3B (in branch, pending merge): members, pending invitations, invite, role change and removal |
...
| `P6-UI-007` | Settings → Organizations & roles | Static row + read-only `Badge` (`Settings.tsx:129-144`); no invite, role change or remove, and no explanatory empty state — unlike the Workspaces card directly beneath it, which does have `+ New` | Looks like team management, does nothing. A pilot user hunts for the invite button and finds none | **UI-P1** | `P6-AUTH-1` (backend implemented in 6B-3A; the endpoints exist, no screen uses them) | **6B-3B** — exit condition met in branch, pending merge: Settings now invites, lists members and pending invitations, changes roles and removes members (`pages/settings/OrganizationMembers.tsx`, `pages/settings/Invitations.tsx`); closes when the 6B-3B PR merges |
...
| 9 | Settings | Account read-only, theme, workspace create/switch, runtime status | — | Roles card inert (`P6-UI-007`) — member management implemented in 6B-3B (in branch, pending merge); no password change |
