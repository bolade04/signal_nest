...
| ID | Item | Current state | Evidence | LB? | Dep | Effort | Risk |
|---|---|---|---|---|---|---|---|
| `P6-AUTH-1` | ~~**A customer cannot add a second user**~~ — **CLOSED in 6B-3A + 6B-3B**: 6B-3A backend merged (#181); 6B-3B customer UI merged (#182 — approved head `dc2973453666cfab0505b821d19dc7d93e27d268`, squash merge `079528dd7dcddc746ca7e33180ffcfa2d554ae22`) with green post-merge CI (run `36224141182`, `push`, all six contexts that ruleset `18820692` requires `success` on `079528d` — see E7, §19). ⚠ **Closing `P6-AUTH-1` does not close 6B**: `P6-AUTH-2`, `P6-AUTH-4` and `P6-AUTH-5`, and the 6B UI surfaces `P6-UI-003` and `P6-UI-017`, remain open. ~~**OPEN on `main`: 6B-3A backend merged (#181); 6B-3B customer UI implementation complete in branch `feat/phase-6b3b-multiuser-ui`, pending merge — closes only when that PR merges with green post-merge CI**~~ | ~~`register()` is the only non-seed `OrganizationMember(` write. No invite, member-add, or role-assignment endpoint exists. 6 roles defined; only `OWNER` is ever assigned, so 5 are unreachable~~ — **6B-3A backend implemented**: an `OWNER` or `ADMIN` invites an email address with one of the five non-`OWNER` roles (`POST`/`GET`/`DELETE /organizations/{organization_id}/invitations`, exact `OWNER`/`ADMIN`). The one-time token is returned once, in the creation response, for manual sharing — there is no email transport — and only its SHA-256 digest is stored; it never appears in a list, an audit row or a log. The invitee previews it (`POST /auth/invitations/preview`, non-consuming) and either registers through it (`POST /auth/invitations/register` — creates the user and the membership, never an organization) or, already holding an account, accepts it while signed in with the invited address (`POST /auth/invitations/accept`). Expiry is 72 hours on the database clock (`invitation_expire_hours`). Acceptance is one-time and transactional (conditional claim), re-checks the inviter's authority, and is serialized by an organization-row lock. Any member lists members (`GET …/members`); `OWNER`/`ADMIN` change roles (`PUT …/members/{user_id}/role`) and remove members (`DELETE …/members/{user_id}`) under the privilege ceiling: only an `OWNER` grants, changes or removes `OWNER`, nobody changes or removes themselves, and the last `OWNER` can be neither demoted nor removed. `OWNER` is never an invitation role (schema and service). OpenAPI 90 → 99 operations; one additive migration (`3dc124a7dfd7`). The API authorization matrix — including the `P6-AUTH-3` audit-route case — is proven on memberships created only through real invitations, with a policy table checked for completeness against the mounted routes. ~~**`P6-AUTH-1` remains OPEN** pending the 6B-3B customer UI (`P6-UI-007`) and role-aware mutation-control remediation (`ROLE_GATED_UI_REMEDIATION_DEFERRED_TO_6B3B`, below)~~ — **6B-3B (merged, #182, `079528d`)** ~~**(implementation complete in branch, pending merge)**~~: an `OWNER`/`ADMIN` invites from Settings and receives the one-time link `<origin>/invite#token=…` to copy and share manually (the UI states that no email is sent); the public `/invite` page captures the token from the URL fragment, scrubs it from the address bar and router, keeps it only in component memory, previews the invitation, and lets a new user register or a signed-in user with the invited address accept (a different signed-in account is never auto-accepted); the session is refreshed and the joined organization becomes active. Settings lists members, pending invitations (revocable, never re-showing a link), and offers role change and removal only where the API would allow them. All role policy lives in `apps/web/src/lib/roles.ts`, pinned to the backend rank. The customer flow — new account and existing account — is proven through the UI without seeding the second membership (`apps/web/src/pages/__tests__/e7-customer-flow.test.tsx`). ~~**`P6-AUTH-1` stays OPEN on `main` until that PR merges with green post-merge CI**~~ — that condition is met: #182 merged as `079528d`, and post-merge CI run `36224141182` passed all six ruleset-required contexts on that SHA, so **`P6-AUTH-1` is CLOSED on `main`** | `apps/api/app/organizations/invitations.py`; `apps/api/app/organizations/members.py`; `apps/api/app/organizations/routes.py:118-206`; `apps/api/app/auth/routes.py:85-114`; `apps/api/alembic/versions/20260924_1030-3dc124a7dfd7_add_organization_invitations.py`; tests `test_organization_invitations_api.py`, `test_invitation_acceptance_api.py`, `test_organization_members_api.py`, `test_e7_role_policy_matrix.py`, `test_organization_invitation_database.py` (migration, invariants and PostgreSQL concurrency); 6B-3B (#182, merge `079528d`): `apps/web/src/pages/invite/InvitePage.tsx`, `apps/web/src/pages/invite/invite-token.ts`, `apps/web/src/pages/settings/OrganizationMembers.tsx`, `apps/web/src/pages/settings/Invitations.tsx`, `apps/web/src/lib/roles.ts`; tests `pages/__tests__/e7-customer-flow.test.tsx`, `pages/__tests__/role-control-matrix.test.tsx`, `pages/invite/__tests__/invite-page.test.tsx`, `pages/settings/__tests__/members-settings.test.tsx`, `lib/__tests__/roles.test.ts` | **Yes** | none | M | ~~SignalNest is single-seat; no company can use it~~ — multi-user at the API; ~~still single-seat in the product until 6B-3B, because no screen invites, lists or manages members, and~~ since 6B-3B merged (#182, `079528d`) the product invites, lists and manages members, and the controls below are rendered only for the roles the API accepts (editor actions for `OWNER`/`ADMIN`/`MARKETER`, Settings `+ New` for `OWNER`/`ADMIN`; non-editors keep every read surface, including a read-only location view) — historical `ROLE_GATED_UI_REMEDIATION_DEFERRED_TO_6B3B` finding: eleven customer mutation controls render for every role although the API refuses the roles outside their gate — opportunity status (`pages/OpportunityDetail.tsx:87`) · onboarding submit (`pages/Onboarding.tsx:146`) · campaign-context create (`pages/CampaignContext.tsx:262`) · campaign-context delete (`pages/CampaignContext.tsx:403`) · scout pause (`pages/scouts/useScoutActions.ts:19`) · scout resume (`pages/scouts/useScoutActions.ts:28`) · scout run (`pages/scouts/useScoutActions.ts:37`) · job cancel (`pages/scouts/JobsPanel.tsx:46`) · scout-request create (`pages/scouts/ScoutRequestDialog.tsx:86`) · location update (`pages/locations/LocationDialog.tsx:159`) · location create + Scout Reach upsert (`pages/locations/LocationDialog.tsx:160-161`) — plus Settings → Workspaces `+ New` (`apps/web/src/pages/Settings.tsx:150-154`). ~~A non-`OWNER` member reaching the UI today is offered actions the API then refuses with 403.~~ True until 6B-3B merged; remediated by #182. Disclosed 6B-3A residuals: **`R2` (accepted)** — the role CHECK on `organization_invitations` admits all six roles, so an `owner`-role invitation row can exist, but only through a direct database write (the request schema and the service never create one). Accepting it is refused (409 `invitation_inviter_not_authorized`; no `OWNER` is granted), while previewing it, and listing that organization's invitations while it exists, fail with 500, because the response schemas carry only the five invitable roles so that OpenAPI never advertises `owner`; deleting the row by id still revokes it. **`F2` (product risk)** — addresses are matched exactly, with no case folding, and `users.email` is case-sensitive, so an address differing from an existing account only in local-part case is a different address: registering through an invitation, like ordinary `POST /auth/register`, can create a second, case-variant account. Both residuals are carried forward: the `P6-AUTH-1` closure does **not** close `R2` or `F2` |
...
| `P6-AUTH-3` | ~~Audit-log route over-permits by rank-floor widening~~ — **CLOSED in 6B-1** | ~~`require_role(OWNER, ADMIN, COMPLIANCE_REVIEWER)` computes `min_rank = min(4,3,1) = 1`, so **MARKETER (2) and REVIEWER (1) both pass**. Only VIEWER is denied~~ — **Closed in 6B-1**: the route now uses `require_exact_roles(OWNER, ADMIN, COMPLIANCE_REVIEWER)`, exact membership with no rank widening, so **MARKETER, REVIEWER and VIEWER are all rejected 403**. Proven at the HTTP boundary by a six-role matrix running real dependency resolution (only `get_db` is overridden); the same frozen tests fail against genuine pre-fix `1c71a7e`, where MARKETER and REVIEWER both returned 200. `require_role` itself is unchanged — see `P6-AUTH-6` | `apps/api/app/auth/dependencies.py:140-173`, `:23-30`; `apps/api/app/audit/routes.py:20-22`; `apps/api/app/tests/test_audit_logs_role_matrix.py` | **Yes** | none | S | ~~Live defect in `main`; masked today only because `P6-AUTH-1` means nobody but OWNER exists. Becomes exploitable the moment role assignment ships~~ — closed **before** `P6-AUTH-1` ~~ships~~ shipped role assignment (#181 API, #182 UI), which was the point of sequencing 6B-1 first |
...
| `P6-AUTH-7` | ~~`POST /organizations/{id}/workspaces` has no role gate~~ — **CLOSED in 6B-2** | ~~Membership check only — any member of any rank, including `VIEWER`, can create a workspace~~ — **Closed in 6B-2**: the route now requires exact organization role `OWNER` or `ADMIN` (`require_exact_organization_roles(Role.OWNER, Role.ADMIN)`), so **MARKETER, REVIEWER, COMPLIANCE_REVIEWER and VIEWER are all rejected 403 before any `Workspace` write**. It uses a new organization-scoped exact-role seam in the shared auth module — `OrganizationContext`, `get_organization_context`, `require_exact_organization_roles` — the organization-level counterpart of `require_exact_roles` (`P6-AUTH-6`) for routes that have no workspace: exact set membership only, no rank, no floor, no inheritance. Membership is still resolved server-side from the persisted `OrganizationMember` row of the path organization (the shared `_membership` helper), and the workspace is written to the authorized organization (`ctx.organization.id`). The `GET` routes remain membership-only. Proven at the HTTP boundary by a six-role matrix running real dependency resolution (only `get_db` is overridden), each attempt's committed workspace count read through a separate engine: `OWNER` and `ADMIN` get 201 with delta +1; the other four get 403 with the role-denial message and delta 0. Measured on the base route during discovery, all six roles got 201 and each left a row. Two disclosed behaviour changes on this route: the non-member message is now the shared "You are not a member of this organization." (the `GET` routes keep "Not a member of this organization."), and authorization now precedes body validation, so a forbidden role or non-member sending an invalid body gets 403, not 422 (undecodable JSON is still rejected 422 first) | `apps/api/app/auth/dependencies.py:41-47`, `:111-137`, `:176-207`; `apps/api/app/organizations/routes.py:60-82`; `apps/api/app/tests/test_organization_role_authorization.py`; `apps/api/app/tests/test_workspace_creation_authorization.py` | No | ~~`P6-AUTH-1`~~ — closed **before** `P6-AUTH-1` shipped role assignment; 6B-3A has since implemented role assignment at the API, and ~~`P6-AUTH-1` itself remains open pending 6B-3B~~ `P6-AUTH-1` itself has since closed (6B-3B, #182) | S | ~~Sets the precedent that makes a future tenant-scoping bypass likely; currently latent because only OWNER exists~~ — the route no longer relies on a route-local membership check: it authorizes by exact organization role through the shared seam and writes only to the organization that seam authorized. The Settings → Workspaces `+ New` control (`apps/web/src/pages/Settings.tsx:150-154`) ~~is still~~ was, at 6B-2 closure, still rendered for every role, so a member outside `OWNER`/`ADMIN` would be offered a create action the API now refuses — a UI residual this closure does not address, and since 6B-3A a reachable one (non-`OWNER` members now exist); deferred to 6B-3B under `ROLE_GATED_UI_REMEDIATION_DEFERRED_TO_6B3B` (see `P6-AUTH-1`); remediated in 6B-3B (merged, #182, `079528d`): `+ New` renders only when `canCreateWorkspace` allows it (`OWNER`/`ADMIN`) |
| `P6-AUTH-6` | ~~No exact-set authorization primitive~~ — **CLOSED in 6B-1** | ~~`require_role` is rank-floor only; `require_exact_roles` does not exist~~ — **Closed in 6B-1**: `require_exact_roles(*allowed: Role)` is a general, reusable dependency factory in the shared auth seam, not audit-route-specific logic. Exact membership only (`ctx.role in allowed`); no rank, no inheritance, no floor. Empty allowed-set raises `ValueError` at construction (mirroring `require_role`, whose `min()` does the same); duplicates normalise through a `frozenset` so a repeated role can never broaden access; denial reuses `PermissionDeniedError` with a message identical to `require_role`'s, naming only the requester's own role. Resolves tenancy through the same `Depends(get_tenant_context)` seam, so no request parameter is added and the OpenAPI entry is byte-unchanged. **`require_role` is deliberately left rank-floor** — the audit-log route was the only current call site where rank-floor evaluation admitted roles outside its explicitly named set. The other 21 current call sites use `OWNER`/`ADMIN`/`MARKETER`, whose named set is already identical to the current rank-floor result (`min_rank = 2`), so they show no effective widening. Exact-set and rank-floor authorization answer different policy questions, so the correct repair was to add `require_exact_roles` and migrate the audit route rather than change `require_role` semantics globally; a regression test fails if `require_role` is ever converted | `apps/api/app/auth/dependencies.py:140-173`; `apps/api/app/tests/test_exact_role_authorization.py` | Yes, for approvals | ~~`P6-AUTH-3`~~ — closed together in 6B-1 | S | ~~Blocks any separation-of-duties claim~~ — the primitive now exists and future separation-of-duties surfaces can consume it |
...
| `P6-PRIV-5` | Audit **coverage** is thin (distinct from immutability) | Exactly **25** `record_audit` call sites — the 19 that predate 6B-3A plus 6 added by 6B-3A for the organization invitation and member audits (`apps/api/app/organizations/members.py:250`, `:297`; `apps/api/app/organizations/invitations.py:362`, `:430`, `:466`, `:514`); **zero** on the 27 campaign-context operations or on workspace creation; onboarding has one (`onboarding.completed`, `apps/api/app/brands/routes.py:37`). There is also no frontend consumer — `apps/web/src/api/queryKeys.ts:82` defines an `auditLogs` key with no endpoint function and no caller | `grep 'record_audit('` → 25 (lines containing `record_audit(` under `apps/api/app`, excluding `app/tests/` and the `def` line; 19 before 6B-3A) | **No for the pilot** (`P6-D02`); **P1 — before GA** | none | M | An audit-spine claim fails on **coverage** before it ever reaches immutability |
...
| `P6-CAP-2` | ~~`FeatureFlagsOut` reflects 1 of 3 flags~~ — **CLOSED in 6U-1G** | ~~A client cannot learn scheduling or RSS is dark without probing and receiving a 503. **This is the backend half of `P6-UI-002`** (UI-P0): `SchedulePanel` cannot gate itself because `scout_scheduling_enabled` is not reflected.~~ **Closed in 6U-1G** (commit `8177549`, #176): all three registered raw global flags are now declared and populated. Reflection alone selects no connector and performs no fetch; it does **not** make scheduling resolver-backed — that is `P6-CAP-1`, still open | `apps/api/app/system/routes.py:61`, `:67`, `:73` + `:126-129` | **Yes — via `P6-UI-002`** | none | S | ~~Without it the most prominent unbuilt feature stays clickable and always fails~~ — realized risk, now closed; `SchedulePanel` gates on the reflected flag |
...
`P6-UI-0` — `P6-UI-001`, `P6-UI-002`, `P6-UI-008` (all three **shipped**: 6U-1B / 6U-1G / 6U-1D)
and `P6-UI-010` (**shipped in 6U-1I**).

### 5.1 Four parallel lanes (refined after `P6-UI-0`)

```
Lane U — FOUNDER-VISIBLE PRODUCT / PILOT UX        (repo-only, no deps, start now)
   6U-1  P6-UI-018 start the worker ─┐ SHIPPED 6U-1A (part 1) + 6U-1I (part 2)
         P6-UI-001 error envelope ───┤ SHIPPED 6U-1B
         P6-UI-019 signals key ──────┤ SHIPPED 6U-1C
         P6-UI-020 active-count tile ┤ SHIPPED 6U-1C (by relabel)
         P6-UI-002 + P6-CAP-2 gate ──┤ SHIPPED 6U-1G
         P6-UI-008 demo creds ───────┤ SHIPPED 6U-1D
         P6-UI-010 stale phase copy ─┘ SHIPPED 6U-1I (5 discovered sites)
...
Lane P — PLATFORM CORRECTNESS / AUTH / DATA        (repo-only, no deps, start now)
   6B    P6-AUTH-3 ─► P6-AUTH-6   — BOTH CLOSED in 6B-1 (shared exact-role primitive
         + audit route migrated off the rank floor); P6-AUTH-1 would have unmasked
         P6-AUTH-3, so it was deliberately closed first and can no longer be unmasked
         P6-AUTH-1 — CLOSED: 6B-3A backend MERGED (#181); 6B-3B customer UI +
         role-aware controls MERGED (#182, 079528d), post-merge CI green
...
- `P6-UI-006` is a UI *symptom* of `P6-CAP-1` (Lane P, 6C). Do not patch it in Lane U —
  ...
  §5 for the full reasoning. That reflection shipped in **6U-1H**, so `P6-UI-005` is now
  **CLOSED**; `P6-UI-006` remains open and still depends on `P6-CAP-1`.
...
2. **A cluster of small, cheap UI defects makes shipped-and-working features look broken**
   — chiefly `P6-UI-001`, which *discarded* every backend error message in the product
   (**closed in 6U-1B**; retained because it is what motivated Lane U).

### 5A.3 Route inventory

~~12 declared routes plus a catch-all (`apps/web/src/App.tsx:21-41`)~~ 12 declared routes at
this audit; **13** plus a catch-all since 6B-3B (#182, `079528d`) added the public `/invite`
route (`apps/web/src/App.tsx:22-41`); 8 sidebar entries, one
...
| `/invite` | `pages/invite/InvitePage.tsx` | no | — | — | `POST /auth/invitations/preview` · `/register` · `/accept` | — | **WORKING** (by test) — added in 6B-3B (#182, `079528d`); not part of the 2026-09-19 audit, which ran live |
...
| `/scout-requests/:id` | `pages/ScoutRequestDetail.tsx` | yes | any | — | scout-request, runs, schedule | `scout_scheduling_enabled` | **PARTIAL** — run history empty (`P6-UI-009`, 6U-2). The schedule panel is now gated on `scout_scheduling_enabled` and withholds the mutation affordances while dark (`P6-UI-002` closed in 6U-1G) |
...
| `/settings` | `pages/Settings.tsx` | yes | any | ✅ | `/auth/me`, workspaces, runtime, organization members + invitations (6B-3B) | — | ~~**PARTIAL** — roles card inert (`P6-UI-007`)~~ **WORKING** — `P6-UI-007` closed in 6B-3B (#182, `079528d`; verified by test, not by a live re-run of this audit): members, pending invitations, invite, role change and removal; the account section still has no password change (`P6-UI-017`, open) |
| `/operations` | `pages/operations/Operations.tsx` | yes | **operator** | ✅ (operator) | 11 `/internal/system/*` | — | **WORKING**, but misreports 1 of 3 capabilities (`P6-UI-006`; `P6-UI-005` closed in 6U-1H) |
...
*appears* to exist as a Settings card and is inert.~~ Team/member management shipped in
6B-3B (#182, `079528d`): Settings manages members and invitations, and the public
`/invite` route previews, registers through and accepts an invitation (`P6-UI-007` closed).
...
**View-only** — ~~Organizations & roles~~ *(interactive since 6B-3B, #182: members,
invitations, role change and removal)* · runtime status · notifications bell (permanent
...
~~team invites~~ *(demonstrable since 6B-3B, #182)* · password reset · billing. All are
either Phase 5A–5E (§4.10, excluded by `P6-D01`) or unbuilt account-lifecycle work
(~~`P6-AUTH-1`~~ — closed in 6B-3A + 6B-3B; `P6-AUTH-2`).

### 5A.6 UI gap register

| UI ID | Screen / flow | Current state | User impact | Pilot blocker? | Backend dep | Tranche |
|---|---|---|---|---|---|---|
| `P6-UI-001` | **Every error toast, app-wide** | `api/client.ts:120-123` builds messages from `humanizeValidation(payload)`, which only reads a top-level `detail` key (`:41-62`). The **sole envelope for application errors** is `{"error":{"code","message","request_id"}}` (`app/core/errors.py:212`), emitted by all three registered handlers (`:220,224,229,244`) **including** `RequestValidationError`. No application error carries `detail`, so the fallback `Request failed (${status})` always fires. **Two things the fix must handle:** field-level detail lives at `error.details` (**plural**, `:214`) — read `message` **and** `details`, or every 422 collapses to "Request validation failed"; and `StarletteHTTPException` is **not** registered, so an unmatched route or 405 still returns Starlette's `{"detail":…}` and must keep working | The server's secret-free, well-written messages — "Opportunity feedback is not available yet.", "This request has no schedule." — **never reach a human**. Every failure in the product is a bare HTTP code. — **Closed in 6U-1B** (commit `1cdb59a`, #170): `apps/web/src/api/client.ts` now matches a **closed allowlist** of three envelope shapes (`:41-54`), reading `error.message` and, for 422, `error.details` (`:158`); the unregistered `StarletteHTTPException` `{"detail":…}` shape still works. **Shipped beyond specification:** the field renderer reads only `msg` and `loc` and **drops `input`** (`:103-104`), because for a `missing` error Pydantic returns the whole request body — **including a submitted password** | **UI-P0 — YES** | none | **CLOSED — 6U-1B** |
| `P6-UI-002` | Scout request detail → schedule | `SchedulePanel` mounts unconditionally (`ScoutRequestDetail.tsx:160`) and renders enabled "Schedule daily"/"Schedule weekly" (`apps/web/src/pages/scouts/SchedulePanel.tsx`); backend 503s the POST (`scouting_requests/routes.py:75,389`). Root cause: `scout_scheduling_enabled` is absent from `FeatureFlagsOut`, so the client cannot know | The most prominent unbuilt feature is presented as working. Click → opaque failure (compounded by `P6-UI-001`) → user concludes the product is broken. — **Closed in 6U-1G** (commit `8177549`, #176) together with its backend half `P6-CAP-2`: the panel reads the capability at `apps/web/src/pages/scouts/SchedulePanel.tsx:55` and withholds the mutation affordances while the flag is dark, keeping an existing schedule readable. It also drops a latched `confirmDelete` intent when the gate closes (`:64-67`), so a destructive confirmation cannot spring back open if the capability returns | **UI-P0 — YES** | `P6-CAP-2` | **CLOSED — 6U-1G** |
...
| `P6-UI-005` | Operations → capability overrides (feedback) | Feedback's **enforcement is already correct** — `feedback/routes.py:85-91` decides via `resolve_capability`. The defect is purely the **reflection**: `system/routes.py:126-129` populates `FeatureFlagsOut` from the raw global flag | ~~An operator enabling feedback for one workspace sees "Enabled · Workspace override" on `/operations` and **still sees no feedback UI**, because the client gate reads the global reflection~~ — **fixed in 6U-1H**: the client gate now reads `GET /workspaces/{workspace_id}/feedback-capability`, the workspace-effective decision, resolved through the same single call site the enforcement gate uses | **UI-P1** | **Not `P6-CAP-2`.** `P6-CAP-2` reflects the *raw global* flags, which is what `P6-UI-002` needs; it cannot close this row. The customer reflection must expose the **workspace-effective** feedback state, which needs workspace context `GET /system/capabilities` does not have | **CLOSED in 6U-1H.** The customer reflection is a dedicated workspace-scoped route sharing one `resolve_capability` call site with the gate, so the UI shows what the backend enforces. `P6-UI-006` / `P6-CAP-1` remain open and are unaffected |
...
| `P6-UI-007` | Settings → Organizations & roles | Static row + read-only `Badge` (`Settings.tsx:129-144`); no invite, role change or remove, and no explanatory empty state — unlike the Workspaces card directly beneath it, which does have `+ New` | Looks like team management, does nothing. A pilot user hunts for the invite button and finds none. — **Closed in 6B-3B** (squash merge `079528d`, #182) | **UI-P1** | `P6-AUTH-1` (backend implemented in 6B-3A; ~~the endpoints exist, no screen uses them~~ consumed by the 6B-3B screens; `P6-AUTH-1` closed) | **CLOSED — 6B-3B** — exit condition met ~~in branch, pending merge~~ on `main`: Settings now invites, lists members and pending invitations, changes roles and removes members (`pages/settings/OrganizationMembers.tsx`, `pages/settings/Invitations.tsx`); ~~closes when the 6B-3B PR merges~~ closed by #182 (squash merge `079528d`; post-merge CI run `36224141182` green on all six ruleset-required contexts) |
| `P6-UI-008` | Sign-in | Live demo credentials printed in plain text (`SignIn.tsx:126-128`) plus a one-click "Use demo account" button (`:110-118`). **`SignIn.tsx` carries no environment guard on any of this**, so the constants compile into the production bundle — confirmed, the built `apps/web/dist/assets/index-*.js` contains `demo1234` | Anyone reaching an externally reachable deployment where the demo org was seeded gets a working **OWNER account inside a real tenant**. Blast radius is the demo org — tenant isolation still protects a pilot's own org. **Correction to an earlier draft:** this row previously claimed the account also grants the operator console. That holds only locally — `apps/api/app/db/seed.py:212` sets `demo_is_operator = environment in ("development","test")`, so `is_operator` is **False** in staging/production. The live `is_operator: true` observation came from a dev server. The finding stands on the unguarded credentials alone. — **Closed in 6U-1D** (commit `83e7037`, #172): the demo block sits behind `import.meta.env.DEV` read at render (`apps/web/src/pages/auth/SignIn.tsx:120`), which Vite statically folds away in a production build, so the constants are no longer emitted into `apps/web/dist`. The `seed.py:212` correction above stands unchanged | **UI-P0 — YES for any external exposure** | none | **CLOSED — 6U-1D** |
...
| `P6-UI-010` | **Five customer-visible sites across four files** *(the earlier scope line "Sidebar + auth layout" recorded where the defect was **found**, not where repair was permitted)* | Untrue or internal-only copy at five sites, spanning **three classes** — and the third is the one a naive "strip every `Phase 3`" fix would have left shipping. *(a) Promises of an unbuilt feature:* `sidebar.tsx:71` ("Creative generation arrives in Phase 3") and `pages/OpportunityDetail.tsx:264` ("Creative generation from these angles arrives in Phase 3"). *(b) Internal-vocabulary leaks:* `sidebar.tsx:69` ("Phase 1 &amp; 2") and `pages/auth/AuthLayout.tsx:55` ("Phase 1 &amp; 2 · Scouting workflow…"). *(c) **False present-tense capability claims** — not roadmap numbering at all:* `pages/CampaignContext.tsx:128` claimed brand voice "Guides reasoning tone" and `:209` claimed channel preferences are "Referenced by recommended actions". **Both were false.** `app/opportunities/context.py:17-62` `build_business_context` reads only `BusinessProfile`, `ProductProfile`, `AudienceProfile` and `CompetitorProfile`; `recommended_action` comes from the LLM explanation (`app/llm/prompts.py:36`, stored `app/jobs/pipeline.py:414-415`) and never reads a channel preference. Confirmed by exhaustion: outside CRUD, the model definitions, `db/seed.py` and the `db/models.py` registry import, `BrandVoiceProfile` and `ChannelPreference` have **zero consumers** in `apps/api/app`. All web paths under `apps/web/src/` | Internal roadmap vocabulary on a customer surface, plus two claims that were simply untrue. — **Closed in 6U-1I for the five DISCOVERED sites** (verified 4/4 by `pages/__tests__/chrome-copy-truthfulness.test.tsx`, which also fails 4/4 when the strings are mutated back). The two false claims now read stored-not-used. `apps/api` was swept in the same tranche and is clean of customer-facing **roadmap copy** in response text — the only Phase strings reaching a client are three **operator-only** `/internal/system/*` OpenAPI descriptions (`system/internal_capabilities_routes.py:343`, `:432`, `system/internal_observability_routes.py:274`, mirrored into the generated `apps/web/src/api/schema.d.ts:124`, `:186`, `:338`), deliberately retained and out of scope. **This is NOT a claim that customer-facing roadmap copy is eliminated**, and the backend sweep covered roadmap copy only — it was **not** a capability-truthfulness audit of backend response text, which no tranche has performed. **KNOWN REMAINING — three further customer-visible sites, outside this tranche's authorized file surface and deliberately NOT fixed here:** `components/layout/notifications.tsx:23` ("Live alerts arrive in a later phase" — already carded as `P6-UI-014` and `P6-P5-6`), `pages/NotFound.tsx:12` ("a capability arriving in a later phase" — **not carded anywhere before this tranche**), and `pages/operations/Operations.tsx:80`, which renders `Phase {item.future_activation_phase}` from API data on the operator console. ⚠ **A literal `grep "Phase [0-9]"` detects NONE of these three** — `Operations.tsx:80` interpolates the digit, and the other two say "a later phase" with no digit at all. Any future verification gate relying on that grep alone **under-reports** | **UI-P1** | none | **CLOSED (5 discovered sites) — 6U-1I** |
...
| `P6-UI-018` | **Scout Requests → "Run now"** | `POST .../run` flips the request to `queued` and enqueues a durable job. **The worker is a separate process** (`apps/api/app/jobs/worker.py:756`) and `app/main.py`'s `lifespan` never starts one — `scripts/dev.sh` has **zero** references to it. Verified live: `GET /internal/system/workers` → `{"active_count":0,"workers":[]}`. The toast meanwhile promises "A background job is processing this scout" (`useScoutActions.ts:43`). **There is no in-process fallback on this route** — the run path is durable-only (`scouting_requests/routes.py:284` → `enqueue_scout_request` → `job_store.enqueue`). Note one decoy: the legacy `@register_job("run_scout_request")` on the synchronous `InProcessQueue` (`jobs/pipeline.py:97`, `infra/queue.py:41-46`) is **never called by the run route** and is not a fallback | **The primary action of the product's primary screen is a dead end under `npm run dev`.** The card flips to Queued and stays there forever; retrying is rejected with "Request is already queued or running." The founder will conclude the product is broken. — **THIS ROW IS TWO PARTS, and "start the worker" was a severe undercount of part one.** **Part 1 — durable execution, closed in 6U-1A** (commit `0d3b798`, #169), which repaired **five** distinct defects: (i) `scripts/dev.sh` started only API + web — it now starts a third process and **supervises** all three, failing the set if any one dies (`scripts/dev.sh:73-74`, `:82`, `:84-109`); (ii) handlers were registered only as an **import side-effect** the worker process never triggered — registration is now explicit (`app/jobs/registry.py:136`); (iii) `supported_job_types` was derived from the `JobType` **enum**, so a handler-less worker still reported READY — it now advertises the **registered** set (`app/jobs/worker.py:648`) and **fails closed** when the registry is incomplete (`:597`); (iv) the worker held only a **subset** of the ORM models, so the first flush raised `NoReferencedTableError` — `app.db.models` is now imported to complete `Base.metadata` (`app/jobs/worker.py:41-45`); (v) none of it was observable, because existing tests imported the world before asserting — the new checks run in a **fresh subprocess** (`app/tests/test_worker_bootstrap.py:74`). **Part 2 — customer-visible settlement, closed in 6U-1I**: the scout detail view observes the `queued`/`running` → terminal transition via explicit allowlists (`pages/ScoutRequestDetail.tsx:31`, `:35`), a predicate `refetchInterval` polling 2000 ms only while in flight (`:47`), and a ref-guarded effect that invalidates **only** on an in-flight → terminal transition (`:66-71`) with `exact: true` on the separately cached opportunities key (`:73-75`) — so the run lands without a manual reload. **FULLY CLOSED — both parts** | **UI-P0 — YES for the walkthrough** | none — the worker is started and supervised by `scripts/dev.sh` since 6U-1A | **FULLY CLOSED — 6U-1A + 6U-1I** |
| `P6-UI-019` | Overview / Scout list / Scout detail — "N signals" | Frontend reads `stats.signals_processed` (`Overview.tsx:99`, `ScoutRequests.tsx:120`, `ScoutRequestDetail.tsx:68`); the API returns `scanned` / `noise_filtered` / `signals_analyzed` / `opportunities`. Verified live: `{"scanned":9,"noise_filtered":1,"signals_analyzed":7,"opportunities":3}`. **The correct key is `signals_analyzed`** = `len(normalized)` (`jobs/pipeline.py:301`) — what survived noise **and** dedupe. It must **not** be derived client-side as `scanned − noise_filtered`: 9−1=8 ≠ 7, because one signal was a duplicate | **Every "signals" figure in the product renders 0** despite real values existing. Overview shows "4 noise filtered · 0 signals". — **Closed in 6U-1C** (commit `ad84171`, #171): a shared `apps/web/src/lib/scout-stats.ts` pins the four contract keys in order (`:30-35`) with `signals_analyzed` at `:33`, typed against the generated `RunStats` schema so a contract rename breaks the build. The client-side `scanned − noise_filtered` derivation was correctly **not** used | **UI-P1 — YES** | none | **CLOSED — 6U-1C** |
| `P6-UI-020` | Overview — "Active scout requests" | Counts only `running\|queued\|paused` (`Overview.tsx:94`); all 4 seeded scouts are `completed` | Renders **0** directly beside the hint "4 total". Reads as broken. — **Closed in 6U-1C by RELABEL, not by the recount this plan proposed** (commit `ad84171`, #171). The tile no longer claims "active": it is labelled **"Scout requests"** and shows the **total** (`pages/Overview.tsx:163`), while the queued/running figure moved into the hint as "N in progress" (`:105`). "In flight" is the API's own definition — queued or running; a paused request is deliberately switched off and `POST .../run` refuses it, so it is excluded (`:90-93`). The alternative floated below in §5.3 — counting `completed` into an "active" tile — was **not** taken | **UI-P1** | none | **CLOSED — 6U-1C** |
...
| `P6-UI-016` | Roles vocabulary | Six roles mapped in `lib/labels.ts:162-169`; until 6B-3A only `owner` was ever assigned (6B-3A made every role assignable through the API, so these branches become reachable once a non-`OWNER` member exists — verification ~~is~~ was delivered in 6B-3B), so role-gate `false` branches (`apps/web/src/pages/scouts/SchedulePanel.tsx:150-152`, `:228-233`, driven by `EDITOR_ROLES` at `:22` and `canEdit` at `:52`) ~~are unreachable~~ were unreachable until a non-`OWNER` member could exist — through the API since 6B-3A (#181), through the product since 6B-3B (#182) *(they now sit at `SchedulePanel.tsx:147-149`, `:225-230`, `canEdit` at `:49`, driven by `canEditWorkspace` in `apps/web/src/lib/roles.ts:117-119`)* | ~~Dead presentation code; the UI will never show a non-owner role~~ — **Closed in 6B-3B** (squash merge `079528d`, #182): non-`OWNER` roles are now reachable through the real invitation flow (`e7-customer-flow.test.tsx`), and the SchedulePanel `false` branches are exercised by `schedule-panel.test.tsx` with non-editor session roles; the invitation flow itself does not assert SchedulePanel | **UI-P2** | `P6-AUTH-1` (closed) | **CLOSED — 6B-3B** |
...
# (no separate worker terminal since 6U-1A — scripts/dev.sh starts and supervises it)
```
...
**Both halves of that sentence were made false by 6U-1A** (commit `0d3b798`, #169) and are
struck rather than deleted, because they are the finding that motivated the fix:
`scripts/dev.sh` now carries **12** worker references and starts and supervises the worker
as a third process, and `README.md:49`, `:54`, `:72`, `:74` document it. The primary action
no longer dead-ends under `npm run dev`.

| # | Step | What works | What is simulated | Intentionally unavailable |
|---|---|---|---|---|
| 1 | Sign in | Full | — | No "forgot password" (`P6-UI-003`) |
| 2 | Overview | Stat cards, classification chart, by-market chart, recent opportunities, recent activity | All 12 opportunities, each badged **Simulated** | ~~Two stat values render wrong~~ — **both closed in 6U-1C**: `P6-UI-019` reads `signals_analyzed`, and the "active" tile was **relabelled** "Scout requests" with the in-flight count moved into the hint (`P6-UI-020`) |
...
| 5 | Scout Requests | List 4 completed requests, create a new one. **"Run now" works** — `scripts/dev.sh` starts and supervises the worker (`P6-UI-018` part 1, 6U-1A) | Fixture connectors, flagged in the create dialog | Live sources; edit; delete/archive |
| 6 | Scout detail | Request config, "Simulated sources" banner | Sources | **Run history is empty** (`P6-UI-009`, 6U-2 — still open); schedule mutation affordances are now withheld while the capability is dark (`P6-UI-002` closed in 6U-1G) |
...
| 9 | Settings | Account read-only, theme, workspace create/switch, runtime status; members and invitations (6B-3B, #182) | — | ~~Roles card inert (`P6-UI-007`)~~ — member management shipped in 6B-3B (#182; `P6-UI-007` closed); no password change |
| 10 | Operations *(operator)* | Capability registry, queue health, worker fleet, schedules, telemetry, override read/write | — | Overrides don't bind in the customer UI for 1 of 3 capabilities (`P6-UI-006`; `P6-UI-005` closed in 6U-1H) |
...
is running**. Fix `P6-UI-018`, `P6-UI-001`, `P6-UI-002`, `P6-UI-019` and `P6-UI-020` first:~~
— **all five have since shipped** (6U-1A / 1B / 1G / 1C), and the worker caveat no longer
applies because `scripts/dev.sh` starts and supervises it. The original reasoning is
retained as the record of why Lane U was sequenced first: they were the difference between
"a real product with a dark feature" and "a product that appears broken on its primary
screen". All five were `S`, repo-only, needed no operator input and no AWS, and none of
them touched Phase 5A–5E scope.

⚠ The `UI-READY-2` token above is **deliberately not re-adjudicated** by this
reconciliation, which records closures only. Re-assessing UI readiness — and the three
customer-visible roadmap strings still open at §5A.6 — is separate work under its own
authorization.
...
| Item | One-line fix (as originally proposed) | Outcome |
|---|---|---|
| `P6-UI-018` | Start the worker in `scripts/dev.sh` (or detect zero workers and say so) | **SHIPPED 6U-1A — and it was FIVE fixes, not one** (see §5A.6); part 2 shipped 6U-1I |
| `P6-UI-001` | Read `payload.error.message` in `api/client.ts:120-123` | **SHIPPED 6U-1B** — closed allowlist; also drops `details[].input` |
| `P6-UI-019` | Read `signals_analyzed`, not `signals_processed`, in 3 files | **SHIPPED 6U-1C** |
| `P6-UI-020` | ~~Count `completed` in the Overview "active" tile~~, or relabel it | **SHIPPED 6U-1C — RELABELLED.** The count was **not** changed |
| `P6-UI-002` | Add `scout_scheduling_enabled` to `FeatureFlagsOut`; gate `SchedulePanel` like `FeedbackPanel` | **SHIPPED 6U-1G** (with `P6-CAP-2`) |
| `P6-UI-010` | Replace the stale "Creative generation arrives in Phase 3" chrome | **SHIPPED 6U-1I** — five sites / four files, not two; three further sites remain (§5A.6) |
...
`P6-UI-002`.** *(Closed in 6U-1H by a dedicated workspace-effective route. The reasoning
below is retained because it is why that route exists rather than an extra field on
`FeatureFlagsOut`.)* An earlier draft excluded it alongside `P6-UI-006` on the assumption that
...
| **6U-1** | **Founder-visible UI truthfulness** (added by `P6-UI-0`) — **all seven items now closed** | `P6-UI-001` (6U-1B), `P6-UI-002` + backend half `P6-CAP-2` (6U-1G), `P6-UI-008` (6U-1D), `P6-UI-019` (6U-1C), `P6-UI-020` (6U-1C, by relabel), `P6-UI-018` (part 1 6U-1A, part 2 6U-1I), `P6-UI-010` (6U-1I, five discovered sites). **`P6-UI-005` is NOT 6U-1 content** — it needs a workspace-effective reflection, not the raw-flag one `P6-CAP-2` provides (see §5); it closed separately in **6U-1H** and is **not counted toward this tranche**. ⚠ **Closing 6U-1 does not close Phase 6** — 6B–6G and 6U-2 are untouched and §19 still enumerates the unmet §4 blocker set | No | No | everything — it is repo-only and tiny |
...
also compounded `P6-AUTH-7` into "any member creates workspaces, unlogged" in a pilot that
E0 requires to be multi-user (E7). 6B-2 closed the authorization side — creation now
requires exact organization role `OWNER` or `ADMIN` — so what remains is "an `OWNER` or
`ADMIN` creates workspaces, unlogged": the audit-coverage limb (zero `record_audit` on
workspace creation) stays open under `P6-PRIV-5`. (b) Because
...
7. A second user has been invited to an organization, holds a **non-OWNER** role, and the
   ...
   test. *(6B-3A: `E7_BACKEND_AUTHORIZATION_MATRIX_PROVEN` — the API matrix, including the
   `P6-AUTH-3` case, is proven on memberships created only through real invitations
   (`test_e7_role_policy_matrix.py`). ~~`E7_COMPLETE = false`: the customer cannot yet invite
   or accept through the product (6B-3B), and the role-aware mutation controls are
   deferred.~~) (6B-3B: **E7 implementation evidence complete** ~~**in branch, pending merge**~~ —
   the customer invitation flow is proven through the UI in both acceptance modes, new
   account and existing account, without seeding the second membership
   (`apps/web/src/pages/__tests__/e7-customer-flow.test.tsx`), and every mutation control
   renders only for the roles the API accepts (`role-control-matrix.test.tsx`).
   ~~`E7_COMPLETE` stays `false` on `main` until the 6B-3B PR merges with green
   post-merge CI.~~)*
   **`E7_COMPLETE = true` on `main`.** PR #182 (approved head
   `dc2973453666cfab0505b821d19dc7d93e27d268`) was squash-merged as
   `079528dd7dcddc746ca7e33180ffcfa2d554ae22`. The recorded execution is post-merge CI run
   `36224141182` (`push`, attempt 1), which passed all six CI contexts on that exact SHA:
   `Frontend quality`, `Backend quality`, `Migrations and API contract`,
   `Integration smoke`, `Container build and security`, and `Revision reader (unit, IaC
   contract, in-image)`. Those are exactly the six contexts that ruleset `18820692`
   required when #182 merged (ruleset last updated 2026-09-20, after this plan was
   authored). The `P6-CI-1` row (§4.7), which still records four required contexts, and
   the statements that depend on it (`P6-GOV-6`, `P6-D06`, X5, §15) predate that update
   and are **not** re-adjudicated by this closure. E7 is evidenced
   by tests: the UI half runs in jsdom against the MSW-modelled API
   (`apps/web/src/test/server.ts`), and the API half runs against memberships created
   only through real invitations. No browser E2E run exists (`P6-CI-2`, open), and no
   deployment occurred.
